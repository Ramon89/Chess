...
class KingTest extends FlatSpec {
  ...
  "The king" should "be able to move like the queen, but with only one step" in {
    ...
    assert(!new KingMove(new King(WHITE), Location('E', 3), Location('E', 2)).isValid(b))
    assert(new KingMove(new King(WHITE), Location('E', 3), Location('E', 4)).isValid(b))
    assert(new KingMove(new King(WHITE), Location('E', 3), Location('F', 4)).isValid(b))
  }
}